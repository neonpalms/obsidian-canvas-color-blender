...
export default class CanvasColor {
	...
	constructor(arg: string | undefined) {
		// Uncolored node
		if (arg === undefined || arg === "") this.c = this.UNCOLORED;
		// Hex code supplied
		else if (/#[\dABCDEFabcdef]{6}/.test(arg)) this.c = this.hexToRGB(arg);
		// Obsidian color index supplied
		else if (/^[1-6]$/.test(arg)) this.c = this.indexToRGB(arg);
		// Else, that's invalid input
		else
			throw new Error(
				`Invalid argument ${arg}; should either be a full hex code like '#4000FF', an index '1' thru '6', or empty string literal''`,
			);
	}
	...
	indexToRGB(colorIdx: string): RGBColor {
		...
		if (hexColor == null)
			throw new Error(
				`Invalid color index ${colorIdx}; should be 1 through 6.`,
			);
		else return this.hexToRGB(hexColor);
	}
	...
	static avg(arg: string[] | CanvasNodeData[]): string {
		...
		const colors =
			typeof arg[0] === "string"
				...
				: (arg as CanvasNodeData[]).map(
						(node) => new CanvasColor(node.color),
					);
		...
		colors.forEach((color, index) => {
			...
			colorsForDebug.push(
				`Node ${index + 1}: \x1b[38;2;${color.c.r};${color.c.g};${color.c.b}m${hexColor}\x1b[0m`,
			);
		});
		...
		console.debug(
			`Average Color: \x1b[38;2;${avgColor.r};${avgColor.g};${avgColor.b}m${avgHex}\x1b[0m`,
		);
