...
export default class CanvasColorBlenderPlugin extends Plugin {
	...
	async updateAllNodeColorsBasedOnEdges() {
		...
		connections.forEach(
			(fromNodes: CanvasNodeData[], toNode: CanvasNodeData) => {
				if (fromNodes.length == 1) {
					...
					switch (this.settings.edgeColorOption) {
						case "source":
							...
							break;
						case "black":
							toNode.color = CanvasColor.avg([
								fromNode.color as string,
								"#000000",
							]);
							break;
						case "white":
						...
						case "custom":
							...
							toNode.color = CanvasColor.avg([
								fromNode.color as string,
								this.settings.customColor as string,
							]);
							break;
					}
	...
	async onunload() {
		console.log(
			"%cCanvas color blender plugin unloaded.",
			"color:#4000FF;",
		);
	}
}
...
class CanvasColorBlenderSettingTab extends PluginSettingTab {
	constructor(
		app: App,
		private plugin: CanvasColorBlenderPlugin,
	) {
		super(app, plugin);
	}
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"When making an edge to an uncolored node, treat the uncolored node's color as:",
			)
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value) => {
						this.plugin.settings.edgeColorOption = value as
							| "source"
							| "black"
							| "white"
							| "custom";
	...
	private updateVisibility(colorPickerSetting: Setting) {
		...
		colorPickerSetting.settingEl.style.display = isCustomColor
			? "block"
			: "none";
	}
}
